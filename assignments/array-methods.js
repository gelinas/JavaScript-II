...
// let fullNames = runners.map(name => `${name.first_name} ${name.last_name}`)
...
let runnersLargeSizeShirt = runners.filter(function(justLarge) {
  return justLarge.shirt_size === "L";
});
console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
function totalDonations(accumulator, myDonation) {
  return accumulator + myDonation.donation;
}
let ticketPriceTotal = runners.reduce(totalDonations, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 -- Make an array of objects with contact information
let contactList = runners.map(function(person) {
  let contact = {
...
// Problem 2 -- find rich donors
let richDonors = runners.filter(fatPockets => fatPockets.donation > 200);
console.log(richDonors);

// Problem 3  -- count donations just from rich donors shirts
let richDonorTotal = richDonors.reduce(totalDonations, 0);
console.log(richDonorTotal);